function [posOut,velOut,omegaOut,path,omegaPath,contact] = trajectory(posIn,velIn,omegaIn,rhoAir)
...
tstart=0;
tfinal=1;  %in sec
global stepTime; stepTime = 0.01;

options1=odeset('events', 'on', 'RelTol', 1e-7, 'AbsTol', 1e-8);

[tt,state,te,oute,ie] = ode45('RHSVar', [tstart:stepTime:tfinal],X0,options1,P);
